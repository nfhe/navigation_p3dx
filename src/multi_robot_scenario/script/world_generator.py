...
class worldGenerator:
                    ...
                    static_models.append(
                            ...
                            <link name='link'>
                                <collision name='collision'>
                                    <geometry>
                                        <box>
                                            <size>{ob_size[0]:.1f} {ob_size[1]:.1f} {height:.1f}</size>
                                        </box>
                                    </geometry>
                                    <surface>
                                        <friction>
    ...
    def load_dyanmic_obtacles(self):
        ...
        for obstacle_type in dynamic_obstacles:
            ...
            curr_path = os.path.dirname(os.path.abspath(__file__))
            parent_path = os.path.dirname(curr_path)
            plugin_path = os.path.join(parent_path, "plugins/libobstaclePathPlugin.so")
            ...
            while i < num_obstacles:
                ...
                if (obstacle_type == "box"):
                    ...
                    dynamic_models.append(
                            ...
                            <pose>{ox} {oy} {z_pos} 0 0 0</pose> <!-- X, Y, Z, Roll, Pitch, Yaw -->
                            <link name='link'>
                                <collision name='collision'>
                                    <geometry>
                                        <box>
                                            <size>{ob_size[0]} {ob_size[1]} {height}</size>
                                        </box>
                                    </geometry>
                                    <surface>
                                        <friction>
                            ...
                            <plugin name="obstacle_motion" filename="{plugin_path}">
                                <orientation>false</orientation>
                                <loop>1</loop>
                                <velocity>{velocity}</velocity>
                                <path>
                                <waypoint>{ox} {oy} {z_pos}</waypoint>
                                <waypoint>{gx} {gy} {z_pos}</waypoint>
                                </path>
                            </plugin>
                            <static>0</static>
                            </model> 
